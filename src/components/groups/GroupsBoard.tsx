      ...
      <CardHeader className="gap-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <StatusPill label={`${gameState.revealedGroups.length}/4 solved`} />
            <StatusPill label={`${gameState.selectedIds.length}/4 selected`} />
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Button
            ...
            disabled={gameState.selectedIds.length !== SELECTION_SIZE || isSubmitting}
...
function SelectionTray({ selectedCards }: { selectedCards: Array<{ id: string; text: string }> }) {
  const slots = Array.from({ length: SELECTION_SIZE }, (_, index) => selectedCards[index] ?? null);
    ...
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="text-muted-foreground text-xs font-semibold tracking-[0.14em] uppercase">
          {selectedCards.length}/{SELECTION_SIZE}
        </span>
      </div>
...
function MistakeMeter({ remainingMistakes }: { remainingMistakes: number }) {
  return (
    <div className="mt-2 flex items-center gap-2" aria-label={`${remainingMistakes} mistakes left`}>
      {Array.from({ length: SELECTION_SIZE }, (_, index) => {
