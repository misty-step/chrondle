...
export const ShareButton: React.FC<ShareButtonProps> = ({
  ...
  const getButtonStyles = () => {
    const baseStyles =
      "font-semibold py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center gap-2";
    ...
    >
      ...
      {status === "success" && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="absolute inline-flex h-full w-full animate-ping rounded-lg bg-white opacity-20" />
        </div>
      )}
    </button>
  );
