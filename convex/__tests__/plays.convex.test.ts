...
describe("plays/queries", () => {
  ...
  describe("getUserCompletedPuzzles", () => {
    it("returns all completed puzzles for user", async () => {
      ...
      // BUG: The query filter `q.neq(q.field("completedAt"), null)` doesn't work
      // as expected because `undefined !== null` is true. So all 3 plays are returned.
      // TODO: Fix the query to use a proper filter for optional fields.
